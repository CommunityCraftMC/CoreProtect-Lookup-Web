...
require_once "res/php/ActionDefinitions.class.php";

$config = require "config.php";
$formConfig = $config['form'];
$formConfig['actions'] = ActionDefinitions::all();
$formConfig['actionGroups'] = ActionDefinitions::groups();

function renderActionButtonGroup($group, $actions) {
    foreach ($group as $actionKey) {
        if (!isset($actions[$actionKey]))
            continue;

        $action = $actions[$actionKey];
        $id = htmlspecialchars($action['domId'], ENT_QUOTES, 'UTF-8');
        $label = htmlspecialchars($action['label'], ENT_QUOTES, 'UTF-8');
        echo "<label for=\"$id\" class=\"action-btns btn btn-secondary\">";
        echo "<input type=\"checkbox\" id=\"$id\" data-action-key=\"$actionKey\"> $label";
        echo "</label>";
    }
}
...
<!--    <link rel="stylesheet" href="res/css/jquery-autocomplete.css">-->
  ...
  <body data-spy="scroll" data-target="#row-pages">
    <nav id="top" class="navbar navbar-expand-lg navbar-dark bg-primary mb-3">
      <div class="container">
        <?php $homeHref = isset($config['navbar']['Home']) ? $config['navbar']['Home'] : $config['page']['href']; ?>
        <a class="btn btn-secondary navbar-home" href="<?php echo htmlspecialchars($homeHref, ENT_QUOTES, 'UTF-8') ?>" aria-label="Home">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
          <ul class="navbar-nav mr-auto">
              <?php foreach($config['navbar'] as $link => $href) {
                  if ($link === 'Home')
                      continue;
                  echo '<li class="nav-item"><a class="nav-link" href="' . $href . '">' . $link . '</a></li>';
              }
              ?>
          </ul>
        </div>
...
-->
      ...
      <div class="card mb-3">
        ...
        <form id="lookup-form" class="card-body">
          ...
          <div class="row">
            <div class="col-auto form-group">
              <div class="input-group-append btn-group btn-group-toggle" data-toggle="buttons">
                <?php renderActionButtonGroup($formConfig['actionGroups']['primary'], $formConfig['actions']); ?>
              </div>
            </div>
            <div class="col-auto form-group">
            ...
            <div class="col-auto form-group">
              <div class="btn-group btn-group-toggle" data-toggle="buttons">
                <?php renderActionButtonGroup($formConfig['actionGroups']['messages'], $formConfig['actions']); ?>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-auto form-group">
              <div class="btn-group btn-group-toggle" data-toggle="buttons">
                <?php renderActionButtonGroup($formConfig['actionGroups']['items'], $formConfig['actions']); ?>
              </div>
            </div>
          </div>
    ...
    <script>
      ...
      const config = <?php echo json_encode($formConfig) ?>;
    </script>
